...
object Huffman {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encodeIt(currentTree: CodeTree, currentText: List[Char]): List[Bit] = currentTree match {
      case Leaf(c, w) =>
        if (currentText.tail.isEmpty)
          List()
        else
          encodeIt(tree, currentText.tail)
      case Fork(l, r, c, w) =>
        if (currentText.isEmpty) throw new RuntimeException("Cannot encode empty character")
        else if(chars(l).contains(currentText.head)) 0 :: encodeIt(l, currentText)
        else if(chars(r).contains(currentText.head)) 1 :: encodeIt(r, currentText)
        else throw new RuntimeException("Unknown character=" + currentText.head)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    val found: List[Bit]= table.find(t => t._1 == char).get._2
    if (found == None) throw new RuntimeException("Code for char=" + char + " not found!")
    found
  }

  def convert(tree: CodeTree): CodeTable = {

    def convertWithPath(tree: CodeTree, reversePath: List[Bit]): CodeTable = tree match {
      case Leaf(c, w) => List((c, reversePath.reverse))
      case Fork(l, r, c, w) => mergeCodeTables(convertWithPath(l, 0 :: reversePath), convertWithPath(r, 1 :: reversePath))
    }

    convertWithPath(tree, List())
  }

  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = (a ::: b).sortWith((t1, t2) => t1._1 < t2._1)

  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    val encoding = codeBits(convert(tree))_

    def encode(text: List[Char]): List[Bit] = text match {
      case List() => List()
      case x :: xs => encoding(x) ::: encode(xs)
    }
